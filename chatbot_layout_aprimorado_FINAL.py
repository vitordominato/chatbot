...
def exibir_pdf(nome, descricao, chave_unica):
    file_path = Path(nome)
    if file_path.exists():
        with open(file_path, "rb") as f:
            st.download_button(f"📎 Baixar PDF: {descricao}", f, file_name=nome, key=chave_unica)
...
if submit:
    ...
    if respostas:
        ...
        for idx, (texto, arquivo) in enumerate(respostas):
            st.markdown(f"- {texto}")
            if arquivo:
                chave_unica = f"{arquivo}_{idx}"
                exibir_pdf(arquivo, arquivo.replace(".pdf", ""), chave_unica)
    else:
